...
public class HangmanHelper {
	...
	public static void main(String[] argv) {
		if (argv.length != 2) {
			System.err
					.println("Usage: HangmanHelper <wordlist-file-name> <allowed characters>");
			System.exit(1);
		}
		try (Scanner input = new Scanner(System.in)) {
			while (true) {
				...
				System.out
						.println(emphasized("New Game\nFile name of wordlist: "
								+ wordlistFilename
								+ "\nAllowed characters for guesses: "
								+ allowedCharacters));
	...
	private static void einSpiel(Scanner input, String wordlistFilename,
			String allowedCharactersForGuesses) {
		...
		System.out.println("Loading word list with " + anzahlBuchstaben
				+ " letters...");
		Wordlist wordlist = new Wordlist(
				HangmanHelper.class.getResourceAsStream(wordlistFilename),
				anzahlBuchstaben);

		HangmanGame hangmanGame = new HangmanGame(wordlist,
				allowedCharactersForGuesses, new MinMaxOneStepStrategy());
		System.out.println(wordlist.getRemainingWords().size() + " Words with "
				+ anzahlBuchstaben + " letters loaded.");
		...
		while (true) {
			...
			if (nextInput.equals(".")) {
				System.out.println("The following words are left: "
				...
				if (wordlist.getRemainingWords().size() < 2) {
					if (wordlist.getRemainingWords().size() == 1) {
						System.out.println(emphasized("Solution found after "
								+ nrOfGuess + " guesses: "
								+ wordlist.getRemainingWords().get(0)));
					} else {
						System.out
								.println(emphasized("The solution is not contained in the wordlist"));
					}
					break;
				}
			}
	...
	private static Character doBestGuess(Wordlist wordlist,
			HangmanGame hangmanGame) {
		Character bestGuess = hangmanGame.bestGuess();
		if (bestGuess == null) {
			System.out
					.println("No more differentiating allowed characters left. You have to guess:\n"
							+ wordlist.getRemainingWords());
		} else {
			System.out.println(wordlist.getRemainingWords().size()
					+ " words remaining: "
					+ ((wordlist.getRemainingWords().size() > 20) ? wordlist
							.getRemainingWords().subList(0, 20) + " ... "
							: wordlist.getRemainingWords()));
